-- Creates the AddInsertedSystemToStaging trigger
CREATE TRIGGER AddInsertedSystemToStaging
ON StarSystems
AFTER INSERT
AS
BEGIN
	...
	WHERE NOT EXISTS (
		SELECT 1 FROM StarSystemStaging sss WHERE i.systemID = sss.systemID
	);
END;

-- Creates the AddUpdatedSystemToStaging trigger
CREATE TRIGGER AddUpdatedSystemToStaging
ON StarSystems
AFTER UPDATE
AS
BEGIN
	...
	MERGE INTO StarSystemStaging AS target
    USING (
        SELECT 
            i.systemID, 
            i.isColonised, 
